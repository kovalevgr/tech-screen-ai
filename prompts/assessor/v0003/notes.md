...
- Pre-merge tightening (reviewer finding, 2026-08-03): repeated refusals / «Не знаю» WITHOUT a substantive attempt never earn level 0 — they stay "not assessable" (empty `assessments` array). Level 0 requires quotable substantive failed attempts. Encoded identically in `system.md` §7.3 and the `level-guide.md` Level 0 NOT-list; meta-rule 6 de-scoped from "once" accordingly.

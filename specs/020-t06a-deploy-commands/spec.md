...
- **FR-011** *(amended: satisfied by PR #19's pre-existing script — see research D10)*: System MUST ship `scripts/cloud-sql-power.sh` (`wake`|`sleep`|`status` × `dev`|`prod`) — the operator helper the FR-007 message references; it patches `activationPolicy` and reports instance state, and is never executed by CI.

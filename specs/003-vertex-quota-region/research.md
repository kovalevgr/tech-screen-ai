...
## R1 — Vertex quota mechanics: metric name + default value (revised 2026-04-26)

**Decision (revised after live observation).** Gemini 2.5 GA models on Vertex AI use a **global per-base-model TPM (input tokens per minute)** quota, NOT a per-region per-project RPM (requests per minute) quota. The metric is `aiplatform.googleapis.com/global_generate_content_input_tokens_per_minute_per_base_model` with `base_model` dimension values `gemini-2.5-flash-ga` and `gemini-2.5-pro-ga` (note the `-ga` suffix that does not appear in the public `models/gemini-2.5-{flash,pro}` API path). Observed defaults on `tech-screen-493720` (2026-04-26): Flash = **10,000,000,000 TPM**, Pro = **1,000,000,000 TPM**. The legacy per-region RPM metric `aiplatform.googleapis.com/generate_content_requests_per_minute_per_project_per_base_model` (snake_case in API; the spec's earlier CamelCase form `GenerateContentRequestsPerMinutePerProjectPerModel` was inaccurate) covers Gemini 1.x and TTS variants only — zero entries for our 2.5 GA models.

For PoC scope (3 concurrent sessions ≈ 30 k tokens/min sustained Flash workload, ≪ 5 k tokens/min Pro workload), defaults exceed need by 4–5 orders of magnitude. **No raise request is filed** for 2.5 models in T01a (see Clarifications 2026-04-26 and FR-002 revised).

**Rationale.** GCP does not publish a single stable "default quota" table that survives model-version churn — and as this finding shows, even the *quota model* shifted between Gemini 1.x (per-region RPM) and 2.x (global TPM). The right source of truth is the live `gcloud` readout on the target project. The reproducible command for 2.5-era observations:

```bash
# Authoritative TPM defaults for 2.5 GA models in any region (global metric):
gcloud alpha services quota list \
  --service=aiplatform.googleapis.com \
  --consumer=projects/${PROJECT_ID} \
  --filter="metric=aiplatform.googleapis.com/global_generate_content_input_tokens_per_minute_per_base_model" \
...
for q in json.load(sys.stdin):
    for limit in q.get('consumerQuotaLimits', []):
        for bucket in limit.get('quotaBuckets', []):
            ...
            model = dims.get('base_model', '')
            if 'gemini-2.5' in model.lower() and 'tts' not in model.lower():
                print(f'{model}: {bucket.get(\"effectiveLimit\", \"?\")} TPM (default {bucket.get(\"defaultLimit\", \"?\")})')
"
```

The same command is documented in the quickstart so the log's "Quota observed defaults" values are reproducible.

**Alternatives considered.**

- **Hard-coded defaults in the spec.** Rejected — values drift, and the *quota model* itself drifted (RPM → TPM between 1.x and 2.x). Live observation is the only durable source.
- **Cite the legacy RPM metric for 2.5 anyway.** Rejected (and was the original spec error) — the metric returns no rows for 2.5; pointing to it would mislead a reviewer.

---

## R2 — Quota-request channel (effectively N/A for 2.5 models, retained for reference)

**Status (2026-04-26).** Per Clarifications 2026-04-26, no quota raise is requested for 2.5 GA models because their TPM defaults already exceed PoC need by 4–5 orders of magnitude. This section is retained as forward-looking guidance for any future task (`T01a-v2`, T48a follow-ups, post-pilot scale-up) that *does* need to file a raise — for example, when adding a model that returns to RPM-based quotas, or when sustained TPM crosses the default for the global metric.

**Decision (deferred).** **GCP Console → IAM & Admin → Quotas** is the canonical channel for any future raise. The operator filters by `aiplatform.googleapis.com`, selects the relevant metric (TPM or RPM, as applicable), filters by model + region (or selects the global bucket for TPM), clicks *Edit quotas*, enters the requested value, and submits. The resulting support-case ID is captured and logged. A `gcloud alpha services quota update-quota-override` variant is permitted only as a rehearsal step in a non-Owner account — Ihor (Owner) files the production request via Console for the audit trail.

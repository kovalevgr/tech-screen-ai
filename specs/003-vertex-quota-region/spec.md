...
- **Partial grant.** GCP may approve Flash at 60 rpm but deny Pro at 60 rpm (Pro is usage-gated on some accounts). Partial grants are logged verbatim — the log does not round up. Per the Clarifications 2026-04-24 decision, T01a accepts partial grants that meet each model's workload floor (Flash ≥ 30 rpm, Pro ≥ 5 rpm — see FR-002a); any grant below a floor blocks T01a until the request is re-raised.
...
- **FR-002**: T01a MUST request a quota increase for `aiplatform.googleapis.com` metric `GenerateContentRequestsPerMinutePerProjectPerModel` for both Gemini 2.5 Flash and Gemini 2.5 Pro, targeting approximately 60 requests/minute per model, sized to support at most 3 concurrent interview sessions plus one nightly calibration batch (the PoC scope bound per the implementation plan). A partial grant is acceptable for T01a merge provided each model meets its documented workload floor (see FR-002a); below either floor, T01a is blocked until the request is re-raised and the grant meets the floor.
- **FR-002a**: The quota log MUST record, as explicit per-model floor values, **Flash ≥ 30 rpm** (sized for 3 concurrent interview sessions at the expected Interviewer + Assessor turn rate) and **Pro ≥ 5 rpm** (sized for the once-per-session Pre-Interview Planner plus headroom for back-to-back sessions). Grant values at or above both floors satisfy T01a acceptance; values below either floor block T01a until a re-raise lands. A partial grant that meets the floors but stays below the 60 rpm target does **not** require a `T01a-v2` — the re-evaluation trigger only fires on the Phase-2 concurrency threshold (FR-007).

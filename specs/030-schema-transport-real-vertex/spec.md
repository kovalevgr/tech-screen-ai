...
- Q: Semantics preserved across the taxonomy swap? → A: Yes, mapping pinned in `docs/engineering/vertex-integration.md`: HTTP 429 + 5xx-except-504 + connection-level transport errors → retry (3 attempts under the 30-s wall clock); HTTP 504 / transport timeout (the `DeadlineExceeded` equivalent) → NOT retried → `VertexTimeoutError`; HTTP 400/403 → `ModelCallConfigError`; every other upstream code → `VertexUpstreamUnavailableError`. The SDK's own opt-in retry layer stays OFF (`HttpOptions.retry_options` never set) so the wrapper keeps sole ownership of the 3-attempt budget.

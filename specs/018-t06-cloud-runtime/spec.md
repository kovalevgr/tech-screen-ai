...
- **FR-004**: System MUST create, **per environment**, the database principals the project already assumes: an application role (used by the backend at runtime; subject to the §3 `REVOKE`s applied by migration 0001) and a migrator role (owns DDL), plus an IAM-authenticated database user for the CI flag-sync identity. Passwords/credentials MUST NOT appear in Git or PR text.

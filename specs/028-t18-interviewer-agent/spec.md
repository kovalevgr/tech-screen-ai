...
- Q: The plan names `prompts/interviewer/v0001/schema.json` as T18's contract, but T17 never created it. → A: Closed as a preliminary commit on this branch (`docs(T17-gap)`): the schema file codifies the `system.md` §4 output contract — field names, the 5-move `internal_move_executed` enum, and `additionalProperties: false` verbatim from §4; the 1..1200-char `utterance` bounds are NEW normative backstops derived from guardrail §7 (~80 Ukrainian words), not stated in §4 (recorded in `notes.md`). No prompt text changed, so no new prompt version (prompt-edit playbook: version bumps are for behavioural changes).
...
- Q: What audit artifact records a wrapper-level rejection of a successful call? → A: None at T18 — when `call_model` returns OK but the wrapper rejects `result.parsed` (enum/caps violation), the wrapper-layer trace keeps `outcome=ok` and no wrapper-side artifact marks the rejection. Known seam, accepted here: T21's `turn_trace` must record the wrapper-level outcome so rejected-but-billed calls are distinguishable in audit.
